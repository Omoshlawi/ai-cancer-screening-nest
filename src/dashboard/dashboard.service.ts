...
export class DashboardService {
  ...
  async getAdminSummary(currentUserId: string, filters?: FindScreeningsDto) {
    ...
    if (filters?.screeningDateFrom)
      screeningWhere.createdAt = {
        ...screeningWhere.createdAt,
        gte: new Date(filters.screeningDateFrom),
      };
    if (filters?.screeningDateTo)
      screeningWhere.createdAt = {
        ...screeningWhere.createdAt,
        lte: new Date(filters.screeningDateTo),
      };
    if (filters?.providerId && filters.providerId !== 'all')
      screeningWhere.providerId = filters.providerId;
    ...
    if (filters?.providerId && filters.providerId !== 'all')
      followUpWhere.providerId = filters.providerId;
    if (filters?.screeningDateFrom)
    ...
    const [
      ...
      this.prismaService.healthProvider.count(),
      this.prismaService.client.count(),
      this.prismaService.followUp.count({
        where: { ...followUpWhere, category: undefined },
      }), // Global follow-up count with filters
      this.prismaService.followUp.count({
        where: {
          ...followUpWhere,
          category: undefined,
    ...
    riskScreenings.forEach((s) => {
      ...
      if (interpretation.includes('LOW')) riskCounts.LOW++;
        ...
        interpretation.includes('MEDIUM') ||
        interpretation.includes('MODERATE')
      )
        riskCounts.MEDIUM++;
    ...
    const riskDistribution = [
      {
        name: 'Low Risk',
        value: Math.round((riskCounts.LOW / totalRisk) * 100),
        color: 'oklch(0.5 0.1 190)',
      },
      {
        name: 'Medium Risk',
        value: Math.round((riskCounts.MEDIUM / totalRisk) * 100),
        color: 'oklch(0.7 0.15 80)',
      },
      {
        name: 'High Risk',
        value: Math.round((riskCounts.HIGH / totalRisk) * 100),
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const screeningsCount = recentScreenings.filter((s) =>
        dayjs(s.createdAt).isSame(month, 'month'),
      ).length;

      const monthFollowUps = recentReferrals.filter((f) =>
        dayjs(f.createdAt).isSame(month, 'month'),
      );
      const monthCompleted = monthFollowUps.filter(
        (f) => f.completedAt && dayjs(f.completedAt).isSame(month, 'month'),
      ).length;
      const rate =
        monthFollowUps.length > 0
          ? Math.round((monthCompleted / monthFollowUps.length) * 100)
          : 85; // Default/Mock if no data
    ...
    const systemTotalWork =
      (await this.prismaService.screening.count()) +
        (await this.prismaService.client.count()) || 1;
    ...
    return {
      ...
      chpPerformance: chpsWithStats.map((c) => ({
        name: c.firstName
          ...
          : (c as any).user?.name || 'Unknown Provider',
  ...
  async getChpPerformanceReport(filters?: FindScreeningsDto) {
    ...
    const chps = await this.prismaService.healthProvider.findMany({
      include: {
        user: {
          select: {
            name: true,
    ...
    const systemTotalWork =
      (await this.prismaService.screening.count({ where: screeningWhere })) +
    ...
    return chps.map((c) => {
      ...
      const completedFollowUps = c.followUps.filter(
        (f) => f.completedAt,
      ).length;
      ...
      const performance = Math.round(
        ((totalClients + totalScreening) / systemTotalWork) * 100,
      );

      return {
        name: c.firstName
          ? `${c.firstName} ${c.lastName}`
          : c.user?.name || 'Unknown Provider',
